defmodule Perkle.Eth do
  ...
  def get_block_by_number(number, full) do
    ...
    case __MODULE__.send("eth_getBlockByNumber",[hex_num, full]) do
      {:ok, block} ->
        decoded_block = block
        {:ok, decoded_block}
      {:error, reason} ->
  ...
  def get_block_by_hash(hash, full \\ false) do
    case __MODULE__.send("eth_getBlockByHash",[hash, full]) do
      {:ok, block} ->
        {:ok, block}
      {:error, reason} ->
        {:error, reason}
  ...
  def get_transaction_by_hash(hash) do
    case __MODULE__.send("eth_getTransactionByHash",[hash]) do
      {:ok, txn} ->
        decoded_txn = txn
        {:ok, txn}
      {:error, reason} ->
...
@doc """
  ...
  def get_filter_changes(hash) do
    case __MODULE__.send("eth_getFilterChanges",[hash]) do
      {:ok, logs} ->
        {:ok, logs}
      {:error, reason} ->
        {:error, reason}
  ...
  def uninstall_filter(hash) do
    case __MODULE__.send("eth_uninstallFilter",[hash]) do
      {:ok, res} ->
        {:ok, res}
      {:error, reason} ->
        {:error, reason}
...
@doc """
  ...
  def get_transaction_receipt_by_hash(hash) do
    case __MODULE__.send("eth_getTransactionReceipt",[hash]) do
      {:ok, txn} ->
        decoded_txn = txn
        {:ok, txn}
      {:error, reason} ->
